...
interface NotificationContentProps {
  ...
  onHandleIdeaRequest?: (notificationId: string, senderId: string, accept: boolean) => void;
...
export const NotificationContent = ({
  ...
  switch (type) {
    case 'idea_request':
      return (
        <div className="flex items-center gap-2 flex-wrap">
          <span>
            <span className="font-medium">{senderName}</span> {message || 'solicitó unirse a tu idea'}
          </span>
          {!compact && (
              ...
              {onHandleIdeaRequest && (
                <>
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      onHandleIdeaRequest(notificationId, senderId, true);
                    }}
                    className="h-8 px-2"
                  >
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      onHandleIdeaRequest(notificationId, senderId, false);
                    }}
                    className="h-8 px-2"
                  >
    ...
    case 'idea_accepted':
      return (
        <span>
          <span className="font-medium">{senderName}</span> aceptó tu solicitud para unirte a una idea
        </span>
      );
    case 'idea_rejected':
      return (
        <span>
          <span className="font-medium">{senderName}</span> rechazó tu solicitud para unirte a una idea
        </span>
      );

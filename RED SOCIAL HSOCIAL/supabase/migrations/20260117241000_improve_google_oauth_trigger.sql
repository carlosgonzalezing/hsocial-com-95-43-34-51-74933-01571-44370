-- Improve handle_new_user trigger for better Google OAuth support
...
BEGIN
  ...
  v_provider := COALESCE(NEW.raw_user_meta_data->>'provider', '');
  
  -- For Google OAuth, prioritize Google-specific fields
  ...
  -- For Google OAuth, set default values if not provided
  IF v_provider = 'google' THEN
    INSERT INTO public.profiles (
      ...
      NEW.id,
      v_username,
      COALESCE(NEW.raw_user_meta_data->>'career', 'Estudiante'),
      COALESCE(NEW.raw_user_meta_data->>'semester', '1'),
      COALESCE(NEW.raw_user_meta_data->>'gender', 'other'),
      COALESCE(NEW.raw_user_meta_data->>'institution_name', 'Por especificar'),
      COALESCE(NEW.raw_user_meta_data->>'academic_role', 'Estudiante'),
      COALESCE(
      ...
      updated_at = now();
  ELSE
    -- Original logic for non-OAuth users
    INSERT INTO public.profiles (
      id,
      username,
      career,
